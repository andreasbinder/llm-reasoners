...
def rap_gsm8k(base_model: LanguageModel,
    ...
    mod = retrieve_hyparams["embedding_model"]['type']
    #from tools import RetrievalResult, Retrieval
    if mod == 'clip':
        from tools.retrieval_base import ClipRetrieval as Retrieval, RetrievalResult
        #retrieval_base import ClipRetrieval as Retrieval, RetrievalResult
        ...
        from tools.retrieval_base import MPNetRetrieval as Retrieval, RetrievalResult
        from models.mpnet_model import MPNetEmbedder
        #checkpoint_embedding_model = "sentence-transformers/all-mpnet-base-v2"
        embedding_model = MPNetEmbedder(checkpoint_embedding_model)
    elif mod == 'dpr':
        from tools.retrieval_base import DPRRetrieval as Retrieval, RetrievalResult
        from models.dpr_model import DPREmbedder
        #checkpoint_embedding_model = "sentence-transformers/all-mpnet-base-v2"
        embedding_model = DPREmbedder("facebook/dpr-question_encoder-single-nq-base", "facebook/dpr-ctx_encoder-single-nq-base")
        
    # from tools.retrieval_base import CrossRetrieval as Retrieval, RetrievalResult
    ...
    for key in tqdm(dataset, total=len(dataset),
        ...
        retrieval = Retrieval(
            example = example,
            hyparams = interactive_prompt["actions"]["RETRIEVE"]["hyparams"]
        )

        num_sources = retrieval.get_num_sources()
        avg_num_sources += num_sources
        ...
        search_results = retrieval.search(query, top_k=retrieve_hyparams['top_k'])
        retrieved_data = retrieval.process_search_results(search_results, query)
        ...
        dataset[key]['sources'] = retrieved_data['sources']
    ...
    wandb.summary.update({'avg_num_sources': avg_num_sources/len(dataset)})
