...
def rap_gsm8k(base_model: LanguageModel,
    ...
    def load_webqa_dataset_oracle(path_to_webqa, split, resume, caption_model):
        import json
        ...
        lineidx_file = '/nfs/data2/zhangya/webqa/imgs.lineidx'
        tsv_file = '/nfs/data2/zhangya/webqa/imgs.tsv'
        ...
        for key, value in tqdm(iterator, desc="Loading data"):
            ...
            if value.get('txt_posFacts', []) != []:
                new_value['Facts'] = [
                    facts['fact']
                    for facts in value['txt_posFacts']
                ]
            else:
                ...
                #caption_model.generate_caption(query, image_path)  # Assuming empty text prompt
                new_value['Facts'] = [
                    caption_model.generate_caption(
                        ...
                        )  # Assuming empty text prompt
                    for image in value['img_posFacts']
                ]
    ...
    dataset = load_webqa_dataset_oracle(path_to_webqa, split, resume, caption_model)
    ...
    for key in tqdm(dataset, total=len(dataset),
        ...
        context = [
            RetrievalResult(
                state_type="RETRIEVE",
                retrieved_snippets=example["Facts"]
            )
        ]

        model_input = utils.action_prompt(interactive_prompt, question, context, "ANSWER")
